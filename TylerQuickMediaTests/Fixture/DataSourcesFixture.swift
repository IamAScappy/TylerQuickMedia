...
extension MockKakaoRemoteSourceType {
    ...
    func mockSample() {
        stub(self, block: { mock in
            let vclipSample: KakaoVClipResponse = ResourcesLoader.loadJson("kakaoVclipResponse")
            let imageSample: KakaoImageResponse = ResourcesLoader.loadJson("kakaoImageResponse")
...
extension MockNaverRemoteSourceType {
    var sortSimImage: NaverImageResponse {
        return ResourcesLoader.loadJson("sort_naver_image_sim")
    }
    var sortDateImage: NaverImageResponse {
        return ResourcesLoader.loadJson("sort_naver_image_date")
    }
    ...
    func mockSample() {
        stub(self, block: { mock in
            when(mock.searchImages(any())).then({ req in
                let sample: NaverImageResponse = ResourcesLoader.loadJson("naverResponse")
